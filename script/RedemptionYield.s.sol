...
function deployRedemptionYield(
    ...
    _contractName = "";
    _address = payable(address(new RedemptionYield(admin, ustb, oracle, usdc, maximumOracleDelay, compound)));
}
...
contract DeployRedemptionYield is Script {
        ...
        returns (address payable _address, bytes memory _constructorParams, string memory _contractName)
    {
        ...
        (_address, _constructorParams, _contractName) =
            deployRedemptionYield(ADMIN, USTB, USTB_NAVS_ORACLE, USDC, MAXIMUM_ORACLE_DELAY, COMPOUND);
